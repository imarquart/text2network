...
param_list=product(rs_list,cutoff_list, level_list, depth_list, context_mode_list, max_degree_list, sym_list, keep_top_k_list, ma_list, algo_list, contextual_relations_list,keep_only_tokens_list)

for rs, cutoff, level, depth, context_mode, max_degree, sym, keep_top_k, ma, algo, contextual_relations, keep_only_tokens in param_list:

    filename = "".join(
        [config['Paths']['csv_outputs'], "/AllPosOvrlCluster", str(focal_token), "_max_degree", str(max_degree),  "_algo", str(algo.__name__), "_depth", str(depth), "_conRel", str(contextual_relations),
    ...
    context_cluster_all_pos(semantic_network,focal_substitutes=focal_token, times=times, keep_top_k=keep_top_k, max_degree=max_degree, sym=sym, weight_cutoff=cutoff, level=level,
...
cent = get_top_100(semantic_network=semantic_network, focal_tokens=focal_token, times=times, symmetric=sym, compositional=False,
                   reverse=False)
