...
tfidf_list = [None, "pmi", "nweight", "rel_weight", "pmi_weight"]

param_list = product(rs_list, cutoff_list, level_list, depth_list, context_mode_list, max_degree_list, sym_list,
                     keep_top_k_list, ma_list, algo_list, contextual_relations_list, keep_only_tokens_list, tfidf_list)

for rs, cutoff, level, depth, context_mode, max_degree, sym, keep_top_k, ma, algo, contextual_relations, keep_only_tokens, tfidf in param_list:
    filename = "".join(
        [config['Paths']['csv_outputs'], "/OvrlCluster", str(focal_token), "_max_degree", str(max_degree), "_algo",
         ...
         str(cutoff), "_tfidf", str(tfidf), "_lev", str(level),
         str(sym), "_rs", str(rs)])
