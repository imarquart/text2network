...
logging.info("------------------------------------------------")
for depth, levels, moving_average, weight_cutoff, rev, sym, comp, rs, cluster_cutoff, backout,fadd,alters,f_token in param_list:
    ...
    # weight_cutoff=0
    filename = "".join(
        [config['Paths']['csv_outputs'], "/EgoClusterYOY_", str(f_token),  "_backout", str(backout),"_fadd", str(fadd),"_alters", str(str(isinstance(alters,list))),"_rev", str(rev),"_sym", str(sym), "_norm", str(comp), "_lev",
         str(levels), "_clcut",
