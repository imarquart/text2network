...
tokens = ["ambiguity", "war", "hostility", "competition", "tension", "dispute", "change", "diversity", "struggle",
          "cooperation"]

# rev Comp
prox = yearly_proximities(semantic_network, year_list=years, focal_tokens=focal_tokens, moving_average=moving_average,
                          reverse_ties=False, symmetric=True, compositional=False, backout=False)
...
prox = yearly_proximities(semantic_network, year_list=years, focal_tokens=focal_tokens, alter_subset=tokens,
                          moving_average=moving_average, symmetric=True, reverse_ties=True, compositional=True,
                          backout=False)
prox = semantic_network.pd_format(prox)[0]
prox.reset_index(inplace=True)
filename = "/YOY_prox_sym_" + str(focal_tokens) + "_altertokens" + str(len(tokens)) + "_ma_" + str(
    moving_average) + ".xlsx"
path = config['Paths']['csv_outputs'] + filename
path = check_create_folder(path)
prox.to_excel(path, merge_cells=False)
