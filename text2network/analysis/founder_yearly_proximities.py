...
# rev Comp
prox=yearly_proximities(semantic_network, year_list=years,focal_tokens=focal_tokens, moving_average=moving_average,reverse_ties=True, compositional=True, backout=False)
...
# rev Comp
prox=yearly_proximities(semantic_network, year_list=years,focal_tokens=focal_tokens, moving_average=moving_average,reverse_ties=False, compositional=True, backout=False)
prox=semantic_network.pd_format(prox)[0]
prox.reset_index(inplace=True)
filename="/YOY_prox_comp_"+str(focal_tokens)+"_top"+str(top_k_allyears)+"_ma_"+str(moving_average)+".xlsx"
path = config['Paths']['csv_outputs']+filename
path = check_create_folder(path)
prox.to_excel(path,merge_cells=False)
