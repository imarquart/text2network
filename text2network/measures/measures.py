        ...
        if len(cl['graph'].nodes) > 0 and (cl['level'] == levels or include_all_levels):  # Consider only the last level
            ...
            if len(np.intersect1d(nodes, interest_list)) > 0:
                ...
                if len(proximate_nodes) > 0:
                    ...
                    overall_pagerank_measures = return_measure_dict(overall_cluster_pagerank, prefix="globalPR_")
                    pagerank_measures = return_measure_dict(cluster_pagerank, prefix="withinPR_")
                    ...
                    df_dict = {'Year': year, 'Token': name, 'Token_pr': name_pr, 'Prom_Node_pr': top_node_pr,
                    ...
                    if add_individual_nodes:
                        ...
                        for node in list(proximate_nodes.index):
                            if proxim.reindex([node], fill_value=0)[0] > 0 or node == focal_token:
                                ...
                                df_dict = {'Year': year, 'Token': node, 'Token_pr': node, 'Prom_Node_pr': top_node_pr,
                                           'Prom_Node': top_node, 'Level': cl['level'],
                                           'Cluster_Id': cl['name'],
                                           'Parent': cl['parent'],
    ...
    if year_by_year:
        for year in times:
            ...
            for cl_name in cluster_dict:
                ...
                cluster_measures = return_measure_dict(proximate_nodes)
                rev_cluster_measures = return_measure_dict(rev_proximate_nodes, prefix="rev_")
                overall_pagerank_measures = return_measure_dict(overall_cluster_year_pagerank, prefix="globalPR_")
                pagerank_measures = return_measure_dict(overall_cluster_year_pagerank, prefix="withinPR_")
                ...
                topelements_pr = np.array(overall_cluster_year_pagerank.nlargest(6).index)
                topelements_pr = list(topelements_pr[topelements_pr != focal_token])
                if topelements_pr == []:
                    topelements_pr = ["empty"]
                name_pr = "-".join(topelements_pr[0:5])
                ...
                df_dict = {'Year': year, 'Token': cl_name, 'Token_pr': name_pr, 'Prom_Node_pr': top_node_pr,
                           'Prom_Node': top_node, 'Level': cl['level'],
                           'Cluster_Id': cl['name'],
                           'Parent': cl['parent'], 'Nr_ProxNodes': len(proximate_nodes),
...
def extract_all_clusters(level: int, cutoff: float, focal_token: str,
                         ...
                         mode: Optional[str] = "replacement", occurrence: Optional[bool] = False,
                         seed: Optional[int] = None) -> pd.DataFrame:
    return average_cluster_proximities(focal_token=focal_token, interest_list=interest_list, nw=snw, levels=level,
                                       times=times, depth=depth, context=context,
