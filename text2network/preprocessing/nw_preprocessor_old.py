...
class nw_preprocessor:
    ...
    def preprocess_folders(
        self, folder=None, max_seq=0, overwrite=True, excludelist=None
    ):
    ...
    def preprocess_files(
        self, folder=None, max_seq=0, overwrite=True, ext_year=None, excludelist=None
    ):
        """
        ...
        for file_path in tqdm(
            files, desc="Iterating files in {} ".format(folder), leave=False
        ):
            ...
            check_for_COCA = re.split(self.split_symbol, file_name)
            if check_for_COCA[0] == "w":

                check_for_COCA.reverse()
                file_name = str(self.split_symbol).join(check_for_COCA)
                logging.info(
                    "Old COCA dataset found, renaming to {}...".format(file_name)
                )

            if ext_year is None:
                logging.debug("Loading file %s", file_name)
                year = re.split(self.split_symbol, file_name)[0]
                year = int(year)
                offset = 1
            else:
                offset = 0
            ...
            for idx, sent in enumerate(tokenized_text):
                ...
                transl_table = {ord(x): ord(y) for x, y in zip("‘’´“”–-", "'''\"\"--")}
                sent = sent.translate(transl_table)
                sent = (
                    unicodedata.normalize("NFKD", sent)
                ...
                if idx % 100000 == 0:
                    logging.debug(
                        "Sentence at idx {}, year {}, parameters {}:".format(
                            idx, year, params
                        )
                    )
