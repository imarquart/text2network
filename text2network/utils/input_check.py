...
def input_check( years=None, tokens=None):
    ...
    if years is not None:
        ...
        if not (isinstance(years, ndarray) or isinstance(years, list) or isinstance(years, dict) or isinstance(years, int) or isinstance(years,np.integer)):
            raise AssertionError("Parameter years must be int, list, array or interval dict ('start':int,'end':int). Supplied: {}".format(type(years)))
    if tokens is not None:
        ...
        if not (isinstance(tokens, ndarray) or isinstance(tokens, list) or isinstance(tokens, int) or isinstance(tokens, str)  or isinstance(years,np.integer)):
