...
def get_bert_embeddings(args, bert,tokens,pad_token_id,mask_token_id,device=torch.device("cpu")):
    """
    ...
    # Get predictions
    with torch.no_grad():
        loss, predictions, embeddings,attn= bert(tokens, masked_lm_labels=labels, token_type_ids=segments)
    ...
    # Operate on attention and embeddings
    ...
    attn=attn[eyes.bool(),:]
    embeddings=embeddings[:,eyes.bool(),:].permute([1,0,2])
    # Note attention is now of (nr_tokens,sequence_size+2)
    # because we added <SEP> and <CLS> tokens
    return predictions.to(torch.device('cpu')), attn.cpu(), embeddings.cpu()