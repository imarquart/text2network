...
from NLP.utils.network_tools import load_graph, graph_merge, make_symmetric, save_graph, load_graph_overall
...
def overall_onelevel_clustering(years, focal_token, cfg, cluster_function, sum_folder, num_retain=15, ego_radius=2,
                                network_type="Rgraph-Sum"):
    # Assorted Lists and dicts
    ego_graphs = []  # Dict for year:ego graphs
    cfg=configuration()
    logging.info("Loading graph data.")
    #logging.info("Pruning links smaller than %i" % cfg.prune_min)
    ...
    for cluster in tqdm(cluster_it, desc="Top Clusters", leave=False, position=0):
        ...
        if len(cluster) >= 2:
            ...
            cluster_nodes = cut_list(cluster, node_info, num_retain)

            # We create a dict of nested lists and node info for this year
            cluster_info.update({'nodes': cluster_nodes})
...
def overall_clustering(focal_token, cfg, cluster_function, merge_folder, num_retain=15, ego_radius=2,
                       network_type="Rgraph-Sum-Rev_avg_r", cluster_levels=3):
    # Assorted Lists and dicts
    ego_graphs = []  # Dict for year:ego graphs
    logging.info("Loading graph data.")
    logging.info("Pruning links smaller than %f" % cfg.prune_min)

    graph = load_graph_overall(cfg, merge_folder, network_type)
    graph = prune_network_edges(graph, edge_weight=cfg.prune_min)
    ego_graph = nx.generators.ego.ego_graph(graph, focal_token, radius=ego_radius, center=True, undirected=False)
        ...
        if len(cluster) >= 2:
            ...
            for i in tqdm(range(1, cluster_levels), desc="Sub Clustering", leave=False, position=0):
                ...
                for n_cluster in cluster_nest[i - 1]:
                    cl_subsub = nx.subgraph(cluster_subgraph, n_cluster).copy()
                    n_cluster_it = cluster_function(cl_subsub)
