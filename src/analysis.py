...
for network_type in ["Rgraph-Sum-Rev", "Rgraph-Sum"]:
    ...
    for focal_token in cfg.focal_nodes:
        ...
        filename=''.join([cluster_xls, '/centralities', focal_token, '_', network_type, '.xlsx'])
        if not os.path.exists(cluster_xls):
...
for network_type in ["Rgraph-Sum-Rev","Rgraph-Sum"]:
    ...
    filename = ''.join([cluster_xls, '/', focal_token, '_', network_type, 'w_', str(cfg.cluster_window), '.csv'])
