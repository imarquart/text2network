...
def preprocess_files_lines(text_file, database, MAX_SEQ_LENGTH, char_mult, max_seq=0):
    ...
    :param text_file: text file with all content
    ...
    logging.info("Loading file from %s" % text_file)
    ...
    try:
        ...
        logging.error("Could not load %s" % text_file)
    ...
    if len(text) == 0:
        raise ImportError("Text file found empty!")
