...
if __name__ == "__main__":
    ...
    logger = setup_logger(
        config["Paths"]["log"],
        int(config["General"]["logging_level"]),
        "preprocessing.py",
    )
    ...
    globpath = "".join(
        [check_folder(config["Paths"]["import_folder"]), os.sep, "*", os.sep]
    )
    ...
    if paths == []:
        logging.info(
            "Preprocessor in file mode: No subfolders found in {}".format(
                config["Paths"]["import_folder"]
            )
        )
        preprocessor.preprocess_files(
            config["Paths"]["import_folder"],
            overwrite=bool(config["Preprocessing"]["overwrite_text_db"]),
            excludelist=exclude_list,
        )
    else:
        logging.info(
            "Preprocessing subfolders in {}".format(config["Paths"]["import_folder"])
        )
        preprocessor.preprocess_folders(
            config["Paths"]["import_folder"],
            overwrite=bool(config["Preprocessing"]["overwrite_text_db"]),
            excludelist=exclude_list,
        )
    ...
    logging.info(
        "Preprocessing of folder {} complete!".format(config["Paths"]["import_folder"])
    )
    ...
    for i, row in enumerate(data):
        logging.info(
            "Seq {}, Run_id: {}, Year: {}, p1: {}".format(
                i, row["run_index"], row["year"], row["p1"]
            )
        )
