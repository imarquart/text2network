...
def test_consistent_vocab(get_bert_trainer):
    ...
    tokenizer, missing_tokens, total_tokens = trainer.get_consistent_vocabulary(
        queries
    )
    assert (
        "spitzentransformer" in missing_tokens
    ), "Missing word not found when checking tokenizer consistency"
    assert (
        "fehlworttest" in missing_tokens
    ), "Missing word not found when checking tokenizer consistency"
    assert (
        "spitzentransformer" in tokenizer.vocab
    ), "Missing word not in vocab, even if found"
    assert (
        "fehlworttest" in tokenizer.vocab
    ), "Missing word not in vocab, even if found"


@pytest.mark.usefixtures("get_bert_trainer", "get_training_tokenizer_folder")
def test_training_and_prediction(
    get_bert_trainer, get_training_tokenizer_folder
):
    trainer = get_bert_trainer
    ...
    print(results)
    assert results["eval_loss_"] <= trainer.bert_config.getfloat("loss_limit")
