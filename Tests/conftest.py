...
def get_training_tokenizer_folder(get_bert_trainer):
    ...
    if check_step(token_folder, hash):
        ...
        (
            ...
            missing_tokens,
            total_tokens,
        ) = get_bert_trainer.get_consistent_vocabulary(queries)
...
def Neo4j_Interface(test_config):
    ...
    if res["nr_nodes"] > 0:
        raise ImportError(
            "The provided Neo4j database is not empty. For security reasons, testing is disallowed on non-empty databases!"
        )
    yield neo4j_interface
    tokens, token_ids = get_token_list()
    for token in tokens:
        qry = "".join(
            ["Match (r:word {token:'", token, "'})-[:onto]->(t) DETACH DELETE t"]
        )
